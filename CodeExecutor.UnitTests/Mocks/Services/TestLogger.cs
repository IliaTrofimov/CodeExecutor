...
public class TestLogger<TService>(ITestOutputHelper output) : TestLogger(output, typeof(TService).Name), ILogger<TService>
...
public class TestLogger(ITestOutputHelper output, string serviceName = "") : ILogger
{
    ...
    {
        output.WriteLine($"{DateTime.Now:HH:mm:ss.ff} [{logLevel.ToString().ToUpper()}]{preffix}{formatter(state, exception)}");
